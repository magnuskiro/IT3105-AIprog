...
def pot_odds(table, player):
	call_amount = table.bet - player.bet
	odds = call_amount*1.0/(table.pot + call_amount)*1.0
	print call_amount, table.pot, odds
	if odds > 0.3:
		return "1"
	elif odds < 0.3 and odds > 0.2:
		return "2"
	elif odds < 0.2 and odds > 0.1:
		return "3"
	return "4"
...
def handle_contexts(game):
	print "handling context!!!!!\n==========="
	remaining = game.getRemaining()
	print len(remaining)
	for player in remaining:
		for k,v in player.contexts.iteritems():
			if int(k[0]) == 1:
				continue
			else:
				hand = player.get_hand()
				table_cards = game.table.get_cards()
				table_cards = table_cards[:int(k[0])+1]
				hs = handstrength.tryit(player.hand, game.table.cards, int(k[0]))
				#c = ''.join(map(str, context[0]))
				#print context, player.no, hand, table_cards
				#c += context[1]
				pair = [k, str(hs)]
				#print pair
				if pair[0] in player.c:
					l = player.c.get(pair[0])
					l.append(pair[1])
				else:
					player.c[pair[0]] = []
					l= player.c.get(pair[0])
