...
\section{Code Files}
\begin{itemize}
\item[Main] Main file, input sound file and get the most fitting word returned
\item[Classifier] compares input towards the word base. 
\item[HMM] The model in which we store data.
\item[Data] The data manipulation code. Prepeares the input data to be compared.
\item[fbHMM] Forward and Backward HMMs and calcProb all to get probabilities of one samples equality to another.
\item[Learn] The trainig code. Reads the word files we start with and updates the HMM model to be more precise.  
\end{itemize}
